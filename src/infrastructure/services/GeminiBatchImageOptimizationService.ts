...
Apply changes precisely as specified above.`;
  ...
  private generateSpecificInstructions(analysis: ImageAnalysis, roomType: RoomType, isFirstImage: boolean): string {
    const instructions = [];
    
    // Only include instructions for issues that actually exist
    if (analysis.lighting.quality === 'poor' || analysis.lighting.brightness === 'too_dark') {
      instructions.push('• Brighten the room evenly - eliminate dark corners and shadows');
    }
    ...
    // Batch consistency
    if (isFirstImage) {
      instructions.push('• Apply professional real estate photography style as reference for batch consistency');
    } else if (analysis.batch_consistency?.needs_consistency_filter) {
      instructions.push('• Maintain consistent lighting style and color temperature with previous images');
    }
